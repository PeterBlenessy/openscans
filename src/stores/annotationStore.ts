...
interface AnnotationState {
  ...
  /** Delete all measurement + ROI annotations for an instance (keeps markers) */
  clearMeasurementsForInstance: (sopInstanceUID: string) => void
...
export const useAnnotationStore = create<AnnotationState>()(
  persist(
    (set, get) => ({
      ...
      clearMeasurementsForInstance: (sopInstanceUID) =>
        set((state) => ({
          annotations: state.annotations.filter(
            (ann) =>
              !(
                ann.sopInstanceUID === sopInstanceUID &&
                (ann.type === 'measurement' || ann.type === 'region')
              )
          ),
        })),
