 ...
 */
/**
 * Drop duplicate annotations by id, keeping the last occurrence (so a re-added
 * / updated annotation wins). Guards against duplicate React keys downstream
 * and against persisted duplicates accumulating in localStorage — e.g. from
 * re-running AI detection / MR segmentation, which produces deterministic ids.
 */
...
export const useAnnotationStore = create<AnnotationState>()(
  persist(
    (set, get) => ({
      ...
      setAnnotations: (annotations) => set({ annotations: dedupeById(annotations) }),
    ...
    }),
    {
      ...
      // Clean up any duplicate ids already saved in localStorage (from before
      // addAnnotations deduped) so they don't keep rehydrating and producing
      // duplicate React keys / duplicated markers.
      onRehydrateStorage: () => (state) => {
        if (state) state.annotations = dedupeById(state.annotations)
      },
    }
  )
)
