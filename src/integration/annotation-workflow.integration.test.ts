...
import type { MarkerAnnotation, MeasurementAnnotation, RegionAnnotation } from '@/types/annotation'
...
describe('Annotation Workflow Integration', () => {
  ...
  describe('Annotation creation and retrieval', () => {
    it('should create marker annotation and retrieve it', () => {
      ...
      // Add annotation
      useAnnotationStore.getState().addAnnotations([marker])
      ...
      const instanceAnnotations = useAnnotationStore.getState().getAnnotationsForInstance(
    ...
    it('clearMeasurementsForInstance removes measurements/ROIs but keeps markers', () => {
      const uid = mockInstance.sopInstanceUID
      const marker: MarkerAnnotation = {
        id: 'm1', type: 'marker', sopInstanceUID: uid, seriesInstanceUID: 's1',
        instanceNumber: 1, position: { x: 1, y: 1 }, label: 'L1', severity: 'normal',
        description: '', createdAt: new Date().toISOString(), createdBy: 'user', autoGenerated: false,
      }
      const measurement: MeasurementAnnotation = {
        id: 'len1', type: 'measurement', sopInstanceUID: uid, seriesInstanceUID: 's1',
        instanceNumber: 1, points: [{ x: 0, y: 0 }, { x: 10, y: 0 }], measurementType: 'length',
        value: 10, unit: 'px', label: '', createdAt: new Date().toISOString(), createdBy: 'user',
      }
      const region: RegionAnnotation = {
        id: 'roi1', type: 'region', sopInstanceUID: uid, seriesInstanceUID: 's1',
        instanceNumber: 1, points: [{ x: 0, y: 0 }, { x: 5, y: 5 }], shape: 'rectangle',
        description: 'ROI', createdAt: new Date().toISOString(), createdBy: 'user',
      }
      useAnnotationStore.getState().setAnnotations([marker, measurement, region])

      useAnnotationStore.getState().clearMeasurementsForInstance(uid)
    ...
    it('should create measurement annotation and calculate length', () => {
      ...
      useAnnotationStore.getState().addAnnotations([measurement])
    ...
    it('should create multiple annotations for same instance', () => {
      ...
      useAnnotationStore.getState().addAnnotations(annotations)
  ...
  describe('Annotation updates', () => {
    it('should update annotation description', () => {
      ...
      useAnnotationStore.getState().updateAnnotation(marker.id, updated)

      const retrieved = useAnnotationStore.getState().annotations[0] as MarkerAnnotation
  ...
  describe('Annotation deletion', () => {
    it('should delete single annotation', () => {
      ...
      // Delete annotation
      useAnnotationStore.getState().deleteAnnotation(marker.id)
    ...
    it('should delete all annotations for instance', () => {
      ...
      useAnnotationStore.getState().deleteAnnotationsForInstance(mockInstance.sopInstanceUID, false)
    ...
    it('should delete only AI annotations when autoOnly is true', () => {
      ...
      useAnnotationStore.getState().deleteAnnotationsForInstance(mockInstance.sopInstanceUID, true)
