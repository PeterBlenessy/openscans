...
describe('AI Workflow Integration', () => {
  ...
  describe('AI vertebra detection workflow', () => {
    it('should run detection, clear old AI annotations, and add new ones', async () => {
      ...
      vi.mocked(mockDetector.detectVertebrae).mockResolvedValue({
      ...
      annotationStore.addAnnotations([oldAiAnnotation])
      ...
      const { result } = renderHook(() =>
        useAiOperations({
          ...
          setDetecting: vi.fn(),
          setAnalyzing: vi.fn(),
          addAnnotations: annotationStore.addAnnotations,
          deleteAnnotationsForInstance: annotationStore.deleteAnnotationsForInstance,
          addAnalysis: vi.fn(),
      ...
      expect(mockDetector.detectVertebrae).toHaveBeenCalledWith(mockInstance)
      ...
      expect(newAnnotations[0].label).toBe('L1')
    })
    ...
    it('should preserve manual annotations when running detection', async () => {
      ...
      annotationStore.addAnnotations([manualAnnotation])
    ...
    it('should handle detection errors gracefully', async () => {
      ...
      vi.mocked(mockDetector.detectVertebrae).mockRejectedValue(
  ...
  describe('AI radiology analysis workflow', () => {
    it('should run analysis and store results', async () => {
      ...
      const analysisStore = useAiAnalysisStore.getState()
      const errorHandler = renderHook(() => useErrorHandler()).result.current
      ...
      const { result } = renderHook(() =>
        useAiOperations({
          ...
          addAnalysis: analysisStore.addAnalysis,
          handleError: errorHandler.handleError,
        })
      )
    ...
    it('should run detection then analysis on same instance', async () => {
      ...
      vi.mocked(claudeDetector.analyzeImage).mockResolvedValue({
        analysis: {
          ...
          impression: 'Normal spinal alignment',
        },
        processingTimeMs: 2000,
      })
      ...
      expect(annotationStore.annotations[0].label).toBe('L1')
      expect(analysisStore.analyses[0].findings).toBe('Vertebrae detected successfully')
    })
  })
  ...
  describe('Guard conditions', () => {
    it('should not run detection when already detecting', async () => {
      ...
      expect(mockDetector.detectVertebrae).not.toHaveBeenCalled()
    })
