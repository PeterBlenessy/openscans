...
describe('Annotation Workflow Integration', () => {
  ...
  describe('Annotation creation and retrieval', () => {
    ...
    it('should upsert (replace, not duplicate) annotations re-added with the same id', () => {
      // Re-running detection/segmentation produces deterministic ids; adding
      // them again must replace, not append (which caused duplicate React keys).
      const make = (label: string): MarkerAnnotation => ({
        id: 'mr-seg-series-1-L5-8',
        type: 'marker',
        sopInstanceUID: mockInstance.sopInstanceUID,
        seriesInstanceUID: 'series-1',
        instanceNumber: 1,
      ...
      useAnnotationStore.getState().addAnnotations([make('L5')])
      useAnnotationStore.getState().addAnnotations([make('L5-updated')])

      const all = useAnnotationStore.getState().annotations
      expect(all).toHaveLength(1)
      expect((all[0] as MarkerAnnotation).label).toBe('L5-updated')
    })

    it('should drop duplicate ids in setAnnotations (e.g. rehydrated persisted dupes)', () => {
      const dup = (label: string): MarkerAnnotation => ({
        id: 'mr-seg-series-1-L1-4',
        type: 'marker',
        sopInstanceUID: mockInstance.sopInstanceUID,
        seriesInstanceUID: 'series-1',
        instanceNumber: 1,
      ...
      // Simulate a persisted array that already contains duplicates.
      useAnnotationStore.getState().setAnnotations([dup('a'), dup('b')])

      const all = useAnnotationStore.getState().annotations
      expect(all).toHaveLength(1)
      expect((all[0] as MarkerAnnotation).label).toBe('b') // last wins
    })
